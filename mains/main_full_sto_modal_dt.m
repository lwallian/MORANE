function main_full_sto_modal_dt(nb_modes,threshold,type_data,nb_period_test)
...
if plot_modal_dt
    [rate_dt, ILC_a_cst,pchol_cov_noises] = fct_cut_frequency_2_full_sto( ...
        bt_tot,ILC_a_cst,param,pchol_cov_noises);
...
if nargin > 3
    param.N_test = ceil(5*nb_period_test/param.dt);
...
if strcmp(param.type_data, 'incompact3d_wake_episode3_cut')
    T_period = 4.1;
    nb_period = 9;
    param.N_test = ceil(T_period*nb_period/param.dt);
    warning(['simulation on only ' num2str(nb_period) ' periods'])
%     param.N_test = ceil(10*5/param.dt);
%     warning('simulation on only 10 periods')    
end
