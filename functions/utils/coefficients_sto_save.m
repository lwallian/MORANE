...
phi_m_U = phi_m_U(:,1:m,:);
phi_m_U = reshape(phi_m_U, [MX, m d]);
...
for i=1:m
    for j=1:m
        div_f_temp = zeros(MX);
        for k=1:d
            grad_phi_jk = grad_phi(:,:,:,k,j); % Mx x My x d
            ...
            div_f_temp = div_f_temp + dtot_f1.*phi_m_U(:,:,k,i); % Mx x My
        end
...
for i=1:m
    for j=1:m
        for k=1:d
            F2(j,i) = F2(j,i) + 0.5 * integration_mat(reshape((div_z(:,:,1).*grad_phi(:,:,1,k,j) + ...
                div_z(:,:,2).*grad_phi(:,:,2,k,j)).*phi_m_U(:,:,k,i), [1 1 MX]), dX);
        end
    end
end
