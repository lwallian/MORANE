...
%     param_ref.name_file_U_temp={};
    ...
    %%
    warning([ 'The function fct_c_POD has been modified to compute ' ...
        'quicker the covariance matrix c with HRLES3900. ' ...
        'Replace fct_c_POD.m by fct_c_POD_save.m for a ' ...
        'normal using.']);
    
%     param_ref_save = param_ref;
%     param_ref_temp_save = param_ref_temp;
% %     param_temp_save = param_temp;
%     param_blocks_save = param_blocks;
    if ~ strcmp(param_ref.type_data,'inc3D_HRLESlong_Re3900_blocks');
        error('Replace fct_c_POD.m by fct_c_POD_save.m');
    end
%     load('mat_c_partially_computed_inc3D_HRLESlong_Re3900_blocks.mat');
    load('mat_c_partially_computed_inc3D_HRLESlong_Re3900_blocks.mat',...
        'big_Ti','big_Tj','c');
    
%     param_ref = param_ref_save; clear param_ref_save;
%     param_ref_temp = param_ref_temp_save; clear param_ref_save;
% %     param_temp = param_temp_save; clear param_ref_save;
%     clear param_ref_save;
%     param_blocks = param_blocks_save; clear param_blocks_save;
%     clear param
    
    param_ref_temp_temp = param_ref_temp;
    
    param_ref_temp_temp.type_data = [param_ref.type_data num2str(1)];
            
    % Load the param file
    if isfield(param_ref_temp_temp.data_in_blocks,'type_whole_data') 
        % if data are saved in several files
        param_temp=read_param_data(param_ref_temp_temp.type_data, ...
            param_ref_temp_temp.folder_data, ...
            param_ref_temp_temp.data_in_blocks.type_whole_data);
    else
        param_temp=read_param_data(param_ref_temp_temp.type_data, ...
            param_ref_temp_temp.folder_data);
    end
    param_ref_temp_temp=rmfield(param_ref_temp_temp,'type_data');
    param_ref_temp_temp=rmfield(param_ref_temp_temp,'folder_data');
    param_temp = mergestruct(param_temp,param_ref_temp_temp);
...
%     big_Tj_begin = 1;
    ...
    for big_Ti = big_Ti_begin:nb_blocks % loop on files
    %for big_Ti=1:nb_blocks % loop on files
        ...
        for big_Tj = big_Tj_begin:nb_blocks % loop on files
            big_Ti
            big_Tj
            tic;
            % To know if you are at the beginning of the loop
