function [bt_evol, db_fv, db_m, eta, Mi_ss, spiral] = evol_correlated_no_pchol(I, L, C, ...
...
[eta, Mi_ss, spiral] = evolve_sto_coeff(noise_xi, noise_theta, tau, eta, Mi_ss, spiral, n, nb_pcl, dt);
...
function [db_eta, db_Mi_ss, db_Gr] = evolve_sto_coeff(noise_xi, noise_theta, tau, eta, Mi_ss, Gr, n, nb_pcl, dt)

% Evolve both eta_i and M_i_ss
db_eta = evolve_eta(noise_theta, tau, eta, n, nb_pcl, dt);
[db_Mi_ss, db_Gr] = evolve_Mi_ss(noise_xi, tau, Mi_ss, Gr, n, nb_pcl, dt);
...
db_sto = bsxfun(@times, db_spiral, mi_ss_noise); % attention
db_sto = sum(db_sto, 3);
...
noises_xi = permute(noises_xi, [1, 3, 4, 2]);
noises_theta = theta_theta * randn((n + 1) * n, nb_pcl) * sqrt(dt);
noises_theta = permute(noises_theta, [1, 3, 4, 2]);
